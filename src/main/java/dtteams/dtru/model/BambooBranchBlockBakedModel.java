...
import dtteams.dtru.block.BambooBranchBlock;
...
@OnlyIn(Dist.CLIENT)
public class BambooBranchBlockBakedModel implements IDynamicBakedModel {
...
//    protected final ResourceLocation modelLocation;
    ...
    protected final TextureAtlasSprite leavesTexture;
    ...
    public BambooBranchBlockBakedModel(IGeometryBakingContext customData, ResourceLocation barkTextureLocation, ResourceLocation ringsTextureLocation, ResourceLocation leavesTextureLocation,
...
//        this.modelLocation = modelLocation;
    ...
    private void initModels() {
        for (int i = 0; i < 8; i++) {
            ...
            cores[0][i] = bakeCore(radius, Axis.Y, barkTexture, false); //DOWN<->UP
            cores[1][i] = bakeCore(radius, Axis.Z, barkTexture, false); //NORTH<->SOUTH
            cores[2][i] = bakeCore(radius, Axis.X, barkTexture, false); //WEST<->EAST

            rings[i] = bakeCore(radius, Axis.Y, ringsTexture, true);
        }
        leaves = bakeLeaves(leavesTexture);
    }

    public BakedModel bakeLeaves(TextureAtlasSprite leavesTexture){
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull RandomSource rand, @Nonnull ModelData extraData, @Nullable RenderType renderType) {
        ...
        if (numConnections == 0 && forceRingDir != null) {
            ...
            if (sourceDir != null && sourceDir.getAxis() == Axis.Y &&
                    state.hasProperty(BambooBranchBlock.LEAVES) && state.getValue(BambooBranchBlock.LEAVES)) {
                quadsList.addAll(leaves.getQuads(state, Direction.EAST, rand, extraData, renderType));
                quadsList.addAll(leaves.getQuads(state, Direction.WEST, rand, extraData, renderType));
            }
        }
