...
@OnlyIn(Dist.CLIENT)
public class EucalyptusBranchBlockBakedModel implements IDynamicBakedModel {
...
//        this.modelLocation = modelLocation;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (radius < 8) {
                for (Direction dir : Direction.values()) {
                    sleeves[dir.get3DDataValue()][i] = bakeSleeve(radius, dir, barkTexture, barkOverlayTexture);
                }
            }
            cores[0][i] = bakeCore(radius, Axis.Y, barkTexture, barkOverlayTexture); //DOWN<->UP
            cores[1][i] = bakeCore(radius, Axis.Z, barkTexture, barkOverlayTexture); //NORTH<->SOUTH
            cores[2][i] = bakeCore(radius, Axis.X, barkTexture, barkOverlayTexture); //WEST<->EAST

            rings[i] = bakeCore(radius, Axis.Y, ringsTexture, barkOverlayTexture);
        }
    }
    ...
    protected BlockElement generateCorePart (int radius, Axis axis, boolean flipNormals, int tintIndex){
    ...
    public BakedModel bakeCore(int radius, Axis axis, TextureAtlasSprite bark, TextureAtlasSprite overlay) {
