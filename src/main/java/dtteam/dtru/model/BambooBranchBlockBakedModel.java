...
public class BambooBranchBlockBakedModel implements IDynamicBakedModel {
    ...
    public BambooBranchBlockBakedModel(ResourceLocation modelLocation, ResourceLocation barkTextureLocation, ResourceLocation ringsTextureLocation,
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull RandomSource random, @Nonnull ModelData extraData, @Nullable RenderType renderType) {
        ...
        if (side == null && state != null) {
            ...
            if (numConnections == 0 && forceRingDir != null) {
                ...
                if (coreRadius == radii[0] && numConnections == 1
                        //&& state.getValue(BambooBranchBlock.ORIGIN).getAxis().isHorizontal()
                ) {
    ...
    protected int getRadius(BlockState blockState) {
        // This way works with branches that don't have the RADIUS property
