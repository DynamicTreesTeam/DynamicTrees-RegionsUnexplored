...
public class EucalyptusBranchBlockModelGeometry extends BranchBlockModelGeometry {
    ...
    public BakedModel bake(IGeometryBakingContext context, ModelBaker modelBaker, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelState, ItemOverrides itemOverrides) {
        boolean useThickModel = this.useThickModel(this.setFamily(ResourceLocation.tryParse(context.getModelName())));
        if (!useThickModel) {
            return new EucalyptusBranchBlockBakedModel(context, ResourceLocation.tryParse(context.getModelName()), this.barkTextureLocation, this.ringsTextureLocation, this.overlayTextureLocation, spriteGetter);
        } else {
            if (this.thickRingsTextureLocation == null)
                this.thickRingsTextureLocation = this.ringsTextureLocation.withSuffix("_thick");
            return new ThickEucalyptusBranchBlockBakedModel(context, ResourceLocation.tryParse(context.getModelName()), this.barkTextureLocation, this.ringsTextureLocation, this.overlayTextureLocation, this.thickRingsTextureLocation, spriteGetter);
        }
    }
